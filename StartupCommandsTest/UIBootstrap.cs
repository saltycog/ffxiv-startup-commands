    ...
    internal class UIBootstrap
    {
        public static unsafe void Inititalize(IPluginUIMock pluginUI)
        {
            ...
            }))
            {
                ...
                void OnWindowOnSdlEvent(ref SDL_Event sdlEvent)
                {
                    if (sdlEvent.type == SDL_EventType.SDL_KEYDOWN
                        && sdlEvent.key.keysym.scancode == SDL_Scancode.SDL_SCANCODE_ESCAPE)
                    {
                        scene.ShouldQuit = true;
                    }
                }

                scene.Window.OnSDLEvent += OnWindowOnSdlEvent;
                
                SetupFullscreenStyle(scene);
